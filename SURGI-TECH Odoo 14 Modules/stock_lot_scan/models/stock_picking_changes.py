...
class stock_picking_inherit(models.Model):
    ...
    def get_line_by_barcose(self):
        ...
        if self.ean13 != '':
            ...
            list = []
            serial = self.ean13
            lot_obj = self.env['stock.production.lot'].search([('name', '=', serial),('company_id','=',self.env.company.id)])
            item_exist = False
            for rec in self:
                        ...
                        if product_id in move_lines.product_id.ids:
                            ...
                            for lot in lots:
                                lot_id = rec.env['stock.production.lot'].search(
                                    [('name', '=', lot['lot_name']), ('product_id', '=', product_id),('company_id','=',self.env.company.id)])
