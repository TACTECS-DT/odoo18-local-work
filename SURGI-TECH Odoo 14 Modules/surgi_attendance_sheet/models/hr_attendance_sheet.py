...
class AttendanceSheet(models.Model):
    ...
    tot_shift_allowance = fields.Float(compute="_compute_sheet_total",
                                       ...
                                       store=True)
    helpoverlap=fields.Html(string="Help")
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            self.env['attendance.sheet.line'].search([('getReviewed','=',False),('att_sheet_id','=',att_sheet.id)]).unlink()
            ...
            for day in all_dates:
                ...
                if contract.multi_shift:
                    ...
                    if attendance_intervalx:
                        ...
                        ramdanshift=[]
                        normalshift=[]
                        for rshift in contract.shiftslist:
                            if rshift.is_ramdan_shift:
                                ramdanshift.append(rshift)
                            else:
                                normalshift.append(rshift)
                        if self.useramdanshifts and day >= self.ramdan_from and day <= self.ramdan_to:##Ramdan Start
                            #raise UserError(day)
                            currentshift=ramdanshift
                        else:
                            currentshift=normalshift
                        for shift in currentshift:  # get random shifts from contract
...
class resource_inhert(models.Model):
    ...
    overlaptime = fields.Float(string="Overlap Time", default=0.0,help="Overlap Description")
    maxdelay = fields.Float(string="Max Delay", default=0.0,help="Overlap max delay")
    maxcheckin = fields.Float(string="Max CheckIn", default=0.0,help="Overlap max Check in")
    helpoverlap=fields.Html(string="Help")
