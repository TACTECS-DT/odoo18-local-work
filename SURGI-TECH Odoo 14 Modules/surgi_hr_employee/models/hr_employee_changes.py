...
class HrEmployeeBaseDate(models.AbstractModel):
    ...
    social_ins_no_unformatted = fields.Char(
        string="Social Ins No.",
        compute='_compute_social_ins_no_unformatted' ,store=True,readonly=False
    )
    @api.depends('social_ins_no')
    def _compute_social_ins_no_unformatted(self):
        for record in self:
            record.social_ins_no_unformatted = str(record.social_ins_no)
    ...
    grade_year_text = fields.Char(string="Graduation Year", compute='_compute_grade_year_text', inverse='_inverse_grade_year_text', store=True, readonly=False)

    # @api.depends('x_grduation_year')
    def _compute_grade_year_text(self):
        for record in self:
            if record.x_grduation_year:
                record.grade_year_text = record.x_grduation_year.strftime('%Y')
            else:
                record.grade_year_text = False
    def _inverse_grade_year_text(self):
        for record in self:
            if record.grade_year_text:
                try:
                    year = int(record.grade_year_text)
                    record.x_grduation_year = datetime(year, 1, 1)
                except ValueError:
                    record.x_grduation_year = False
            else:
                record.x_grduation_year = False
