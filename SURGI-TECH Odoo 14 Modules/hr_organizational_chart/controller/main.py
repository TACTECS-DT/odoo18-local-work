...
class EmployeeChart(http.Controller):
    ...
    @http.route('/get/dep/employees', type='json', auth='public', method=['POST'], csrf=False)
    def get_dep_employee_ids(self,dep_id,loc_id=''):
        #employees = request.env['hr.employee'].sudo().search([('parent_id', '=', False)])
        if dep_id:
            dep = request.env['hr.department'].sudo().search([('id', '=', int(dep_id))])
            employees=dep[0]['manager_id']
        else:
            employees = request.env['hr.employee'].sudo().search([('parent_id', '=', False)])    
        #raise UserError(str(dep))
        names = []
        key = []
        if len(employees) == 1:
            key.append(employees.id)
    ...
    def get_nodes(self, child_ids,loc_id=''):
        if child_ids:
            child_nodes = """<tr>"""
            for child in child_ids:
                if loc_id =='' or child.work_location == loc_id: 
    ...
    def get_parent_child(self,loc_id='', **post):
        if post:
            ...
            if loc_id!= '':
                child_ids.filtered(lambda r: r.work_location == loc_id)
                #for child in child_ids:
                 #   if child.work_location!=loc_id:
                  #      child_ids.remove(child)
                print("f")
                #child_ids[:]=[x for x in child_ids if x.work_location == loc_id]
            #emp=request.env['hr.employee'].sudo().search([('id','in',post)])
            emp = request.env['hr.employee'].sudo().browse(val)
            table = """<table class='empchartclass'><tr><td colspan='""" + str(len(child_ids) * 2) + """'><div class="node">"""
            view = """ <div id="parent" class='o_chart_head'><a>
                ...
                <p>""" + str(emp.name ) + """</p>
                <p>""" + str(emp.job_id.name) +"<br/>"+str(emp.company_id.name)+"<br/>"+str(emp.department_id.name)+"<br/>"+str(emp.grade_id.name)+"<br/>"+str(emp.rank_id.name)+"<br/>"+str(emp.section_id.name)+"<br/>"+str(len(child_ids))+ """</p>
                
                
                </div></a></div>"""
            table += view + """</div></td></tr>"""
            #emp.count_man_employee
            loop_len = len(child_ids)*2
            lines = self.get_lines(loop_len)
            
            
                #for ch in child_ids:
                 #   if ch.work_location != loc_id:
                  #      child_ids.remove(ch)
            nodes = self.get_nodes(child_ids,loc_id)
            table += lines + nodes
            return table
