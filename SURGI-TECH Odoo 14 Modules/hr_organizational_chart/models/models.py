...
class OrganizationalChart(models.Model):
    ...
    def get_departments(self):
        ...
        deps={}
        vals=self.env['hr.department'].search([('manager_id','!=',False)])
        for i in vals:
            deps[i['id']]=i['name']
        return deps
    @api.model
    def get_locations(self,dep_l_id):
        print("get Departments")
        locations=[]
        #vals=self.env['hr.employee'].search([('manager_id','!=',False)])
        #for i in vals:
         #   deps[i['id']]=i['name']
        #return deps
        #raise UserError(str(dep_l_id)) 
        #locations=self.env['hr.employee'].read_group(domain=[
         #       ('department_id', '=', dep_l_id['dep_l_id'])], fields=['work_location'], groupby=['work_location'])
        sql="select distinct(work_location) from hr_employee "#where department_id="+str(dep_l_id['dep_l_id'])
        #raise UserError(sql)
