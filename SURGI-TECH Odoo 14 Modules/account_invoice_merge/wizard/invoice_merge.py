...
class InvoiceMerge(models.TransientModel):
    ...
    def merge_invoices(self):
        ...
        allinvoices = invoices.do_merge(
            keep_references=self.keep_references, date_invoice=self.date_invoice,performa_seq=self.merge_performa_sequence()
        )
        xid = {
    ...
    def merge_performa_sequence(self):
        inv_obj = self.env["account.move"]
        ids = self.env.context.get("active_ids", [])
        my_seq = 'performa'
        invoices = inv_obj.browse(ids)
        for rec in invoices:
            if rec.partner_id.create_performa_sequence and rec.account_performa_sequence:
                my_seq += rec.account_performa_sequence.split("UPA",1)[1]
        return my_seq