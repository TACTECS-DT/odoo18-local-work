...
class AttendanceSheet(models.Model):
    ...
    def _compute_access_url(self):
        super(AttendanceSheet, self)._compute_access_url()
        for request in self:
    ...
    contract_id = fields.Many2one('hr.contract', string='Contract',
                                  readonly=True,
                                  states={'draft': [('readonly', False)]})
    def get_is_random_shift(self):
        if self.contract_id:
            if self.contract_id.random_shift:
                self.is_rondom_shift=True
            else:
                self.is_rondom_shift=False
        else:
            self.is_rondom_shift=False

    is_rondom_shift=fields.Boolean(string="Is Radom Shift",compute='get_is_random_shift')
    #### Ramdan                              
    useramdanshifts=fields.Boolean("Use Ramdan Shifts",default=False) 
    ramdan_from=fields.Date(string='Ramdan From',
                            default=lambda self: fields.Date.to_string(
                                date.today().replace(day=1)), )# readonly=True,
    ramdan_to= fields.Date(string='Ramdan To',
                          default=lambda self: fields.Date.to_string(
                              (datetime.now() + relativedelta(months=+1, day=1,
                                                              days=-1)).date()))# readonly=True,
    # ramdan_schadual=fields.Many2one(comodel_name="resource.calendar",string="Ramdan Schadual")                                                             
    ...
    def _get_emp_leave_intervals(self, emp, start_datetime=None,
        ...
        for leave in leave_ids:
            date_fromx = leave.date_from #Ahmed test
            date_from=date_fromx.astimezone(pytz.timezone(emp.tz)).replace(tzinfo=None)
            if end_datetime and date_from > end_datetime:
                continue
            date_to = leave.date_to
            if start_datetime and date_to < start_datetime:
                continue
            leaves.append((date_fromx, date_to))
        return leaves
...
class AttendanceSheetLine(models.Model):
    ...
    #Ahmed  Edit
    def action_req_overtime(self, **kwargs):
        for rec in self:
            
            att_sheet = rec.att_sheet_id
            tz = pytz.timezone(att_sheet.employee_id.tz)
            timeoffisset=tz._utcoffset.seconds
            duration=0
            if rec.status in ['ab','leave']:
               raise UserError('You Cant make overtime in absence or leave day')
            elif rec.status in ['weekend','ph']:
                duration=rec.ac_sign_out-rec.ac_sign_in
                #pytz.utc.localize(attendance_interval[0]).astimezone(tz)
                # signin = att_sheet._get_float_from_time(
                #                 pytz.utc.localize(
                #                     rec.ac_sign_in).astimezone(tz)).replace(tzinfo=tz).astimezone(pytz.utc).replace(tzinfo=None)
                # signin=pytz.utc.localize(datetime.strptime(str(rec.date)+" "+str(timedelta(hours=rec.ac_sign_in)).rsplit(':', 1)[0],"%Y-%m-%d %H:%M")).astimezone(pytz.utc).replace(tzinfo=None)
                # sigout=pytz.utc.localize(datetime.strptime(str(rec.date)+" "+str(timedelta(hours=rec.ac_sign_out)).rsplit(':', 1)[0],"%Y-%m-%d %H:%M")).astimezone(pytz.utc).replace(tzinfo=None)
                signin=datetime.strptime(str(rec.date)+" "+str(timedelta(hours=rec.ac_sign_in)).rsplit(':', 1)[0],"%Y-%m-%d %H:%M")-timedelta(seconds=timeoffisset)#.replace(tzinfo=None)
                #sigout=datetime.strptime(str(rec.date)+" "+str(timedelta(hours=rec.ac_sign_out)).rsplit(':', 1)[0],"%Y-%m-%d %H:%M")-timedelta(seconds=timeoffisset)#.astimezone(tz).replace(tzinfo=None))
                sigout=signin+timedelta(hours=rec.act_overtime)
            elif rec.ac_sign_in :
                duration=rec.ac_sign_out-rec.pl_sign_out
                # signin=pytz.utc.localize(datetime.strptime(str(rec.date)+" "+str(timedelta(hours=rec.pl_sign_out)).rsplit(':', 1)[0],"%Y-%m-%d %H:%M")).astimezone(pytz.utc).replace(tzinfo=None)
                # sigout=pytz.utc.localize(datetime.strptime(str(rec.date)+" "+str(timedelta(hours=rec.ac_sign_out)).rsplit(':', 1)[0],"%Y-%m-%d %H:%M")).astimezone(pytz.utc).replace(tzinfo=None)
                signin=datetime.strptime(str(rec.date)+" "+str(timedelta(hours=rec.pl_sign_out)).rsplit(':', 1)[0],"%Y-%m-%d %H:%M")-timedelta(seconds=timeoffisset)#.astimezone(tz)
                #sigout=datetime.strptime(str(rec.date)+" "+str(timedelta(hours=rec.ac_sign_out)).rsplit(':', 1)[0],"%Y-%m-%d %H:%M")-timedelta(seconds=timeoffisset)
                sigout=signin+timedelta(hours=rec.act_overtime)

                    
            if rec.overtime < 1:
                raise UserError('You Cant Make less than 1 h overtime')
                
            else:
                data={
                    'employee_id':att_sheet.employee_id.id,
                    'duration_type':'hours',
                    'date_from' : signin.replace(tzinfo=None),
                    'date_to' : sigout.replace(tzinfo=None),
                    'type' : 'cash'

                }
                self.env['hr.overtime'].create(data)
    def action_req_overtime_portal(self, **kwargs):
        for rec in self:
