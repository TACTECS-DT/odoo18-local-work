...
class JobAnalysisEvaluation(models.Model):
    ...
    department_id = fields.Many2one(comodel_name="hr.department", string="Department",related='job_id.department_id', store=True)
    ...
    def filters_employee_manager(self):
        ...
        for emp in employee_rec:
            if emp.parent_id.user_id == self.env.user and not self.env.user.has_group('surgi_job_analysis.hr_admin_permission_group_job_analysis') :
                categ_ids.append(emp.id)
            elif self.env.user.has_group('surgi_job_analysis.hr_admin_permission_group_job_analysis') :
                categ_ids.append(emp.id)
