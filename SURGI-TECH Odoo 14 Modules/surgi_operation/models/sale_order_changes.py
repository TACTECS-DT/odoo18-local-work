...
class sale_order(models.Model):
    ...
    @api.onchange('hospital_id','partner_id')
    def get_onchanges_hospital_id(self):
        for rec in self:
            if rec.hospital_id:
                 if rec.hospital_id.hospital_activation == False:
                     raise UserError("please make the hospital active to work with non cash method")
            if rec.partner_id:
                 if rec.partner_id.is_hospital == True and rec.partner_id.hospital_activation == False:
                     raise UserError("please make the hospital active to work with non cash method")
