...
class EvaluationEvaluation(models.Model):
    ...
    def _compute_access_url(self):
        super(EvaluationEvaluation, self)._compute_access_url()
        for request in self:
            request.access_url = '/hr/evaluation/%s' % (request.id)

    def _get_report_base_filename(self):
        self.ensure_one()
        return '%s %s' % (_('Hr Evaluation'), self.name)
    ...
    def get_total_total(self):
        ...
        for rec in self:
            ...
            rec.total_totals = ((rec.total_competencies * ratio.core_ratio) + (
                        rec.total_function_comp * ratio.function_ratio) + (rec.total_employee_kpi * ratio.kpi_ratio))/3
